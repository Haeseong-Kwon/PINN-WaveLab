...
class TrainingConfig(BaseModel):
    ...
    resolution: int = 51 # For FDTD and visualization
...
async def train_pinn_wave(websocket: WebSocket, config: TrainingConfig):
    ...
    initial_u = torch.exp(-torch.sum(ic_points_spatial**2 / (2 * 0.1**2), dim=1, keepdim=True)) # Gaussian
    ...
    for epoch in range(config.epochs + 1):
        ...
        if config.use_importance_sampling and epoch > 0 and epoch % 1000 == 0:
            ...
            collocation_points = resample_collocation_points_3d(pinn_model, config)
             ...
             collocation_points = torch.cat([
                torch.rand(config.num_collocation_points, 2, device=device) * 2 - 1,
                torch.rand(config.num_collocation_points, 1, device=device) * config.time_domain[1]
            ], dim=1)
        ...
        if epoch % 250 == 0:
            ...
            log_data.update({
                "wavefield_prediction": pinn_wavefield.tolist(),
                "wavefield_ground_truth": fdtd_wavefield.tolist(),
                "time_slice": t_slice,
                "weights": {k: v.item() for k,v in adaptive_weights.items()}
            })
            try:
